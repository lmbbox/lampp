...
DataTable.Editor.display.bootstrap = $.extend( true, {}, DataTable.Editor.models.displayController, {
	...
	"init": function ( dte ) {
		...
		self._dom.close = $('<button class="close">&times;</button>')[0];

		$(self._dom.close).click( function () {
// - BEGIN Patch
//			self._dte.close('icon');
			if (!self._dte.s.processing)
			{
				self._dte.close('icon');
			}
// + END Patch
...
// - BEGIN Patch
		$('div.DTE_Processing_Indicator', append).html( '<i class="icon-spinner icon-spin icon-2x pull-right"></i>' );
// + END Patch
